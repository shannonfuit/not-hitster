# app/jobs/import_spotify_playlist_job.rb
class ImportSpotifyPlaylistJob < ApplicationJob
  queue_as :default

  def perform(user_id:, playlist_url:)
    ...
    client.each_playlist_track(playlist_url) do |item|
      track = item["track"]
      unless track
        skipped += 1
        next
      ...
      unless release_year.positive?
        Rails.logger.warn(
          ...
          "user=#{user.id} track=#{spotify_uuid.inspect} title=#{title.inspect} artist=#{artist_name.inspect} release_date=#{raw_release_date.inspect}"
        )
        skipped += 1
        next
      end
      ...
      if song.changed?
        begin
          ...
          msgs = if e.record.respond_to?(:errors)
    ...
    Rails.logger.info("[ImportSpotifyPlaylistJob] user=#{user.id} created=#{created} updated=#{updated} skipped=#{skipped}")
  end
end
